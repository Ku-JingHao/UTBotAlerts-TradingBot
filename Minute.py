...
WHATSAPP_PHONE_NUMBER = "+601169525122"  # Replace with your phone number

def send_whatsapp_alert(message):
    """Send a WhatsApp alert message without requiring human intervention"""
    if not ENABLE_WHATSAPP_ALERTS:
        logger.info("WhatsApp alerts are disabled. Not sending message.")
        return False
    
    try:
        logger.info(f"Sending WhatsApp alert to {WHATSAPP_PHONE_NUMBER}")
        
        # Use the instant sending method that doesn't require interaction
        kit.sendwhatmsg_instantly(
            WHATSAPP_PHONE_NUMBER, 
            message,
            wait_time=20,  # Reduced wait time
...
# Trading Parameters
TICKER = "CRV/USD"
INTERVAL = "1m"  # Changed from 1h to 1m

# Alpaca API credentials
ALPACA_API_KEY = "PKRYS2AD809A8MVK3QCR"
ALPACA_SECRET_KEY = "hIp7JJhg9kQmuh33Mm7AA42bbaJQwlFYtDxXwru1"
...
def get_historical_data():
    ...
    timeframe = TimeFrame.Minute  # Changed from Hour to Minute
   
    end = datetime.now()
    # For 1-min data, we'll get data for the past day (1440 minutes)
    start = end - timedelta(1)  # Changed from days=10
...
def calculate_roi_summary():
    ...
    print(f"{'=' * 60}")
    logger.info(f"{Fore.CYAN}PERFORMANCE SUMMARY BY 30-MINUTE INTERVALS{Style.RESET_ALL}")
