...
ENABLE_WHATSAPP_ALERTS = True  # Set to False to disable WhatsApp notifications
WHATSAPP_PHONE_NUMBERS = ["+601169525122", "+60133996236", "+60193617647", "+60132042088", "+60122072556"]             

def send_whatsapp_alert(message):
    """Send a WhatsApp alert message to multiple phone numbers without requiring human intervention"""
    ...
    for phone_number in WHATSAPP_PHONE_NUMBERS:
        try:
            ...
            # Use the instant sending method that doesn't require interaction
            ...
            logger.info(f"WhatsApp alert sent to {phone_number}: {message}")
            
            time.sleep(15)  # Add a delay between sending messages
...
# Global variables to track trades and performance in day intervals
trade_history = []
performance_metrics = {}
...
ALPACA_API_KEY = "PKC33TXDJVH7Q47XDL8D"
ALPACA_SECRET_KEY = "8LSEyXT7YR4BVsJ3V29gRhS8WsV6dyaqgHZUOOwj"
...
def execute_trade(side, price):
    ...
    try:
        ...
        # Send WhatsApp notification with trade details
        signal_type = "BUY 🟢" if side == OrderSide.BUY else "SELL 🔴"
        whatsapp_message = (
            f"UT Bot Alert: {signal_type}\n"
            f"Symbol: {TICKER}\n"
            f"Price: ${price:.2f}\n"
            f"Quantity: {quantity:.6f}\n"
            f"Value: ${quantity * price:.2f}\n"
            f"Account Balance: ${cash_left:.2f}\n"
...
def print_detailed_signals(signals_df):
    ...
    for i, row in signals_df.tail(10).iterrows():
        # Format date (include time for day data)
...
def print_signal_update(latest_bar, current_price, current_position, signal_count=1):
    ...
    print(f"{'=' * 60}")
    logger.info(f"1-Day SIGNAL UPDATE #{signal_count} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Format datetime for Day data
...
def run_live_trading():
    ...
    # # Send initial startup notification to WhatsApp
        ...
        while True:
            try:
                ...
                if last_check_date != current_date:
                    # Reset waiting message flag when starting a new day check
                    waiting_message_shown = False
                    ...
                    # Process sell signal
                        ...
                        logger.info("No new trading signals this day")
                    
                    # Record that we checked today
                    last_check_date = current_date
                            
                # Calculate next check time (next day)
                next_check_time = datetime.now() + timedelta(days=1)

                # Show waiting message only once after processing signals and if not already shown
                if last_check_date == current_date and not waiting_message_shown:
    ...
    if trade_history:
        ...
        if current_position > 0:
            ...
            try:
                ...
                logger.info(f"Open position  : {current_position} XRP valued at ${position_value:.2f}")
            except Exception as e:
