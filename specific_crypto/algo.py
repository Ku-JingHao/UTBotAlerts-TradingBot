...
# Trading Parameters
TICKER = "ALGO-USD"  # Yahoo Finance format for Curve DAO Token
INTERVAL = "1d" 
...
def get_historical_data():
    ...
    try:
        ...
        # Change period and interval for daily data
        df = ticker.history(period="60d", interval="1d")  # Get 60 days of daily data
    ...
    try:
        while True:
            try:
                ...
                # Check for new day instead of new hour
                current_day = current_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
                ...
                next_check_time = (current_datetime + timedelta(days=1)).replace(
                    hour=0, minute=0, second=0, microsecond=0
                )
                
                if last_check_day == current_day and not waiting_message_shown:
                    print_waiting_message(next_check_time)
                    waiting_message_shown = True
                
                time.sleep(300)  # Check every 5 minutes instead of 5 seconds
